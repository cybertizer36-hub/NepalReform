...
export function AgendaVoteSection({ agendaId, className, size = "sm" }: AgendaVoteSectionProps) {
  ...
  // Store vote state in localStorage for immediate persistence
  useEffect(() => {
    if (voteData.userVote !== null) {
      localStorage.setItem(`vote_${agendaId}`, voteData.userVote)
    } else {
      localStorage.removeItem(`vote_${agendaId}`)
    }
  }, [voteData.userVote, agendaId])

  // Load initial vote state from localStorage while fetching from DB
  useEffect(() => {
    const storedVote = localStorage.getItem(`vote_${agendaId}`)
    if (storedVote && (storedVote === "like" || storedVote === "dislike")) {
      // This helps prevent flicker while the actual data loads
      // The actual data from DB will override this when it arrives
    }
  }, [agendaId])
